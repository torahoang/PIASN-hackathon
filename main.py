...
def router(db_research_embeddings, db_resume_embeddings, input):
    model_name = ""
    input_embedding = model.encode([input])[0] # encode the user input
    similarities_research = util.cos_sim(input_embedding, db_research_embeddings)[0].cpu().numpy() # get the cosine matrix compare to the research db
    similarities_resume = util.cos_sim(input_embedding, db_resume_embeddings)[0].cpu().numpy()# get the cosine matrix compare to the resume db
